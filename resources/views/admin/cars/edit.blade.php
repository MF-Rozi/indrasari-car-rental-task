...
<div class="mx-auto max-w-4xl space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl dark:text-white">Edit Vehicle: {{ $car->brand }} {{ $car->model }}</h1>
                <p class="text-xs text-slate-500 dark:text-slate-400">Update vehicle parameters, rate, or operational status.</p>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8 dark:border-slate-800 dark:bg-[#1E293B]">
        <form method="POST" action="{{ route('admin.cars.update', $car) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <div class="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-3">
                    <div>
                        ...
                        <input type="text" id="brand" name="brand" value="{{ old('brand', $car->brand) }}" required autofocus
                            class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 focus:border-purple-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 dark:border-slate-700 dark:bg-[#0F172A] dark:text-white @error('brand') border-rose-500 @enderror">
                        @error('brand')
                    ...
                    <div>
                        ...
                        <input type="text" id="model" name="model" value="{{ old('model', $car->model) }}" required
                            class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 focus:border-purple-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 dark:border-slate-700 dark:bg-[#0F172A] dark:text-white @error('model') border-rose-500 @enderror">
                        @error('model')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="license_plate" name="license_plate" value="{{ old('license_plate', $car->license_plate) }}" required
                            class="mt-1.5 block w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 font-mono text-sm font-bold uppercase text-slate-900 focus:border-purple-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 dark:border-slate-700 dark:bg-[#0F172A] dark:text-white @error('license_plate') border-rose-500 @enderror">
                        @error('license_plate')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                        ...
                        <div class="relative mt-1.5">
                            ...
                            <input type="number" id="daily_rate" name="daily_rate" value="{{ old('daily_rate', $car->daily_rate) }}" step="10000" min="10000" required
                    ...
                    <div>
                        ...
                        <input type="number" id="seating_capacity" name="seating_capacity" value="{{ old('seating_capacity', $car->seating_capacity) }}" min="1" max="20" required
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">Current photo and optional replacement (JPEG, PNG, WebP up to 2MB).</p>
                ...
                <div class="mt-4 flex flex-col gap-6 sm:flex-row sm:items-center">
                    ...
                    <div id="image-preview-container" class="flex h-36 w-48 shrink-0 items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-[#0F172A]">
                        @if($car->image_path)
                            <img id="image-preview" src="{{ Storage::url($car->image_path) }}" alt="{{ $car->brand }}" class="h-full w-full object-cover">
                            <div id="image-preview-placeholder" class="hidden text-center p-3">
                                <span class="text-[11px] text-slate-400">No image chosen</span>
                            </div>
                        @else
                    ...
                    <div class="flex-1">
                        <label class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Select a new image to replace current photo:</label>
